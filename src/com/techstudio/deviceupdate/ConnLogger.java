...
public final class ConnLogger 
{
	...
	static
	{
		try
		{
			myLogger 	= initLogger(ConnConfig.getProperty("log.processid"), ConnConfig.getProperty("log.loggerdir"), 
															ConnConfig.getProperty("log.mode"));
			if ( myLogger==null )
				throw new Exception();
		}
		catch (Exception e)
	...
	private static Logger initLogger(String ProcessID, String logdir, String LogMode){
	    ...
	    properties.setProperty("log4j.appender.file.Append", "true");
			...
			properties.setProperty("log4j.appender.file.layout.ConversionPattern", "%d [%t] %-5p - %m%n");
